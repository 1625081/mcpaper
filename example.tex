...
\begin{center}
	...
	\begin{tabular}{|l|c|c|r|}
		\hline
		16091025&郑金阳&15652576761&sanjin@buaa.edu.cn\\
		\hline
		16091013&苏宴生&18710128609&suyansheng@buaa.edu.cn\\
		\hline
		16091040&张畅&15120075879&zccqyz@buaa.edu.cn\\
		\hline
	\end{tabular}
...
考虑问题中给出的范例与模型构建方向，模型应基于并优化区块链机制。去中心化的链式模型中，知识产权体系相关的拥有者，发行者与使用者能够有效地实现信息的传递与共享。同时，区块链模型会对知识产权拥有者的版权信息，授权过程，以及资金转移等各个方面实现有效地记录和追踪。这使得在利益分配时，平等透明的体系可使得各利益相关方明确地了解资金分配的模式，保证安全性。由此，我们提出了基于区块链的去中心化模型。

\subsection{背景介绍}

作为新兴互联网技术，区块链思想已经应用到支付交易，证券管理，数字医疗等多个不同领域。在知识产权保护方面，区块链也经过了多层次，多角度的尝试与实践：mediachain为数字媒体版权提供了有效的保护与回溯机制，并与音乐平台spotify展开合作；voise平台为艺术家与用户之间提供了点对点，去中心化，完全透明的交易分享平台。区块链系统为传统的知识产权领域提供了全新的思路，也解决了其在认证与交易与收益分配过程中一定的模糊性。

而在区块链应用过程中，诸如模型结构的构建，公有链与私有链等机制的选取，共识机制（如PoW，DPoS等）的确立，都会为体系构建产生巨大影响。我们也试图从这些角度优化区块链机制。
...
Mediachain采取IPFS文件系统存储数据，IPFS是一个开源分布式版本文件系统，它的目标是建立全球范围的点对点的文件传输。IPFS的原理是使用根据文件内容加密形成的哈希地址作为通讯地址，使用BitSwap协议传输，传输过程中只需验证文件的哈希是否正确而无需验证发送者的身份。对IPFS系统的详细介绍和可行性分析见后文第4.4节。


\section{IPTC知识产权交易模型}
\subsection{模型综述}
\subsubsection{知识产权交易链模型概览}
...
本文根据区块链的精神，建立了两个去中心化的系统IPFS与IPTC。前者用于对音乐、软件或其他电子作品专利的存储，既方便作品使用者检索与使用，又能为创作者提供知识产权的保护。后者则记录整个系统中发生的用户注册、版权注册、授权行为与交易行为等一系列信息流动。该过程以区块链的形式完成，利用了其“去中心化”与“不可更改”等特性，实现了对知识产权注册与交易对的离散性、高效性的要求，以及为产权的保护与维权提供了强有力的保证。

\subsection{IPFS-Mediachain元数据区块链模型}

该模型负责储存系统中的电子作品。Mediachain是一种基于区块链技术的开源数字媒体版权保护机制。它的核心产品是一个元数据协议，通过它内容创作者可以给自己的作品附加信息，并把该数据打上时间戳放到对应的的区块链里，然后放到InterPlanetary File System（IPFS，吸收了区块链技术的分布式文件系统）上。具有安全、可扩展、低成本的特性。

\subsection{IPTC-知识产权交易模型}

该模型负责记录系统中的信息流动。首先由用户根据需求与共识，创建记录。并通过用户端将记录请求提交至记录缓冲区，向全部服务商广播。根据DPoS共识在服务商中进行见证人选举，确定一位服务商对记录请求进行有效性检验，并将检验通过的记录打包创建区块，在自己的记录时间到来时根据已有的IPTC链将区块添加，并将新链广播。同时，用户可以通过用户端对IPTC链与IPFS进行目标信息的查询。

\subsection{缺陷与不足}
\begin{itemize}
	\item 本模型中许多变量，例如选举人的人数，服务费所占的比例等并没有给出一个合理可信的确定值，仅在模型描述中给出了这些变量。原因是因为时间有限而没有具体地调研与模拟整个过程，无法确定在实际操作中具体的的值为多少。在今后的模型的实践与应用中可以逐步模拟并确认这些值。
	\item 本模型仅能通过区块链机制保证模型理论上的安全性。而对于网络环境本身存在的风险则没有给予考虑，在模型改进中可以将网络风险因素考虑进来，进一步提升系统的安全性。
\end{itemize}
...
本项目根据去中心化的精神，建立了两个去中心化的系统IPFS与IPTC。前者用于对音乐、软件或其他电子作品专利的存储，既方便作品使用者检索与使用，又能为创作者提供知识产权的保护。后者则记录整个系统中发生的用户注册、版权注册、授权行为与交易行为等一系列信息流动。该过程以区块链的形式完成，利用了其“去中心化”与“不可更改”等特性，实现了对知识产权注册与交易对的离散性、高效性的要求，以及为产权的保护与维权提供了强有力的保证。

由于传播数字媒体时往往伴随着较高的下载量和较低的上传量，故在本模型中上传会获得更高的下载速度，同时去中心化的Merkle DAG存储模式节约了大量的储存空间并且整合利用了闲置资源，使得完整的数字媒体以碎片化的形式保存到第三方服务器上，更安全地保护了数字媒体版权，这符合现代数字媒体体积大下载量高的特点。

用户根据需求与共识创建记录，并向全部服务商广播。根据DPoS共识可以快速地在服务商中进行见证人选举并确定一位服务商对记录请求进行有效性检验，普通用户通过SPV机制进行快速支付，这同时符合用户对授权的速度需求和版权所有者对产权保护的安全需求，同时鼓励了更多的发行商参与区块链提供服务，故这种机制是可行的。

综上，本文建立的IPTC知识产权交易模型，实现了数字媒体版权的快速发布、分类授权和安全交易，从而实现了基于区块链的数字媒体版权收益分配，使发行方在交易系统中平等竞争，使用方在交易系统中得到快速授权，制作方在交易系统中便捷地明确收益，开拓了一条应用区块链保护知识产权的新道路。
...
\begin{flushleft}
	...
	[1]Andrea Corbellini.Elliptic.Curve Cryptography: a gentle introduction [EB OL]. http://andrea.corbellini.name
	
	[2] Nakamoto, Satoshi. Bitcoin: A peer-to-peer electronic cash system. [R].2008.
	
	[3]吴健,高力,朱静宁.基于区块链技术的数字版权保护[J].广播电视信息,2016(07):60-62.
	
	[4]Juan Benet.IPFS-Content Addressed,Versioned,P2P Files System [J].arXiv:14073561v1,14 Jul 2014.
	
	[5] [EB OL].http://docs.mediachain.io/
